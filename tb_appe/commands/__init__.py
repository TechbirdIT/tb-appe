"""
Bench commands & runtime patches for the `tb_appe` app.

This module is auto-discovered by Frappe's bench command loader on every bench
invocation (it calls `importlib.import_module("tb_appe.commands")`). We use that
...
uninstalling an app whose name is a substring of another app's `required_apps`
entry, the loop iterates over installed apps and the check spuriously returns
True and prints:

    App <app> is a dependency of hrms. Uninstall hrms first.

...even though `hrms` does NOT actually require that app. We harden the check to
an exact match so uninstalling `tb_appe` (or any app) is never blocked by a
...
def _patch_frappe_remove_app() -> None:
	...
	if original is None or getattr(original, "_tb_appe_dependency_patch", False):
		return
	...
	try:
		...
		click.secho(
			f"[tb_appe] Warning: failed to apply remove_app() dependency-check patch: {exc}",
			fg="yellow",
		)
		return
	...
	if patched is not None:
		patched._tb_appe_dependency_patch = True
