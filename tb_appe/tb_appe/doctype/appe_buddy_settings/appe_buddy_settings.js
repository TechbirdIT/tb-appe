...
frappe.provide("tb_appe.buddy_settings");
...
function add_actions(frm) {
	frm.add_custom_button(__("Test Connection"), () => {
		frappe.call({
			method: "tb_appe.ai.api.test_connection",
			freeze: true,
...
function hydrate_model_options(frm) {
	...
	frappe.call({
		method:
			"tb_appe.tb_appe.doctype.appe_buddy_settings.appe_buddy_settings.get_provider_preset",
...
function browse_models(frm) {
	...
	frappe.call({
		method:
			"tb_appe.tb_appe.doctype.appe_buddy_settings.appe_buddy_settings.list_provider_models",
...
function render_stats(frm, force = false) {
	...
	frappe.call({
		method: "tb_appe.tb_appe.doctype.appe_buddy_settings.appe_buddy_settings.get_stats",
		callback: (r) => {
			const data = r.message || {};
			const t = data.totals || {};
