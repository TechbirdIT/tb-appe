...
frappe.pages["appe-buddy"].on_page_load = function (wrapper) {
	...
	const ui = new tb_appe.buddy.ChatUI(page);
	ui.render();
...
tb_appe.buddy.renderMarkdown = function (raw) {
	if (raw == null) return "";
	let text = String(raw);
	const codeBlocks = [];
...
tb_appe.buddy.renderToolResultTable = function (result) {
...
tb_appe.buddy.ChatUI = class ChatUI {
	constructor(page) {
		this.page = page;
	...
	async boot() {
		let meta = null;
		try {
			meta = await this.call("tb_appe.ai.api.settings_public");
		} catch (e) {
	...
	async load_conversations() {
		...
		try {
			this.conversations = await this.call("tb_appe.ai.api.list_conversations", { limit: 200 });
		} catch (e) {
			this.conversations = [];
			this.$sidebar.html(
	...
	build_conv_row(c) {
		...
		$row.find(".js-rename").on("click", (e) => {
			...
			frappe.prompt(
				...
				(values) =>
					this.call("tb_appe.ai.api.rename_conversation", {
						name: c.name,
						title: values.title,
		...
		$row.find(".js-pin").on("click", (e) => {
			e.stopPropagation();
			this.call("tb_appe.ai.api.pin_conversation", {
				name: c.name,
				pinned: c.pinned ? 0 : 1,
			}).then(() => this.load_conversations());
		});
		$row.find(".js-delete").on("click", (e) => {
			...
			frappe.confirm(__("Delete this chat? This cannot be undone."), () =>
				this.call("tb_appe.ai.api.delete_conversation", { name: c.name }).then(() => {
	...
	async start_new_chat() {
		try {
			const data = await this.call("tb_appe.ai.api.new_conversation", {
				title: "New Chat",
				context: null,
			});
			await this.load_conversations();
	...
	async open_conversation(name) {
		...
		try {
			this.current = await this.call("tb_appe.ai.api.get_conversation", {
				name,
				message_limit: 300,
			});
		} catch (e) {
	...
	append_message_dom(m) {
		...
		if (role === "tool") {
			...
			`);
			const tableHtml = m.tool_result && tb_appe.buddy.renderToolResultTable(m.tool_result);
		...
		const $row = $(`
			<div class="appe-buddy-msg appe-buddy-msg-${role}">
				<div class="appe-buddy-msg-bubble">${tb_appe.buddy.renderMarkdown(m.content || "")}</div>
	...
	async send_current(prefill_message) {
		...
		try {
			const data = await this.call("tb_appe.ai.api.send_message", {
				message: text,
