...
(function () {
	...
	function fetch_settings_and_toggle({ force = false, throttle = false } = {}) {
		...
		try {
			frappe.call({
				method: "tb_appe.ai.api.settings_public",
				type: "GET",
				callback: (r) => {
	...
	function mount() {
		...
		const panel = new tb_appe.buddy.FloatingPanel(drawer);
		...
		window.tb_appe.buddy.floating_panel = panel;
	...
	tb_appe.buddy.FloatingPanel = class FloatingPanel {
		constructor(root) {
			this.root = root;
			this.$drawer = $(root);
		...
		async bootstrap() {
			...
			try {
				const list = await this.call("tb_appe.ai.api.list_conversations", {
					limit: 1,
					status: "Active",
				});
				if (Array.isArray(list) && list.length) {
					this.conversation = list[0].name;
					const data = await this.call("tb_appe.ai.api.get_conversation", {
						name: list[0].name,
						message_limit: 50,
					});
		...
		async send_current() {
			...
			try {
				...
				const conv = await this.call("tb_appe.ai.api.get_conversation", {
