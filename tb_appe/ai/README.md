...
Appe Buddy is the AI assistant baked into the **TB Appe** Frappe app. It is designed
...
├─ tb_appe/page/appe_buddy/          ← Full-page Desk chat at /app/appe-buddy
...
`POST /api/method/tb_appe.ai.api.send_message`
...
| --- | --- | --- | --- |
| POST  | `tb_appe.ai.api.new_conversation`        | `title?`, `context?` | Create a fresh chat |
| GET   | `tb_appe.ai.api.list_conversations`      | `limit?`, `status?` (Active/Archived) | List user's chats |
| GET   | `tb_appe.ai.api.get_conversation`        | `name`, `message_limit?` | Fetch full chat with messages |
| POST  | `tb_appe.ai.api.rename_conversation`     | `name`, `title` | Rename |
| POST  | `tb_appe.ai.api.pin_conversation`        | `name`, `pinned` | Pin / unpin |
| POST  | `tb_appe.ai.api.archive_conversation`    | `name` | Archive |
| POST  | `tb_appe.ai.api.unarchive_conversation`  | `name` | Unarchive |
| POST  | `tb_appe.ai.api.delete_conversation`     | `name` | Permanently delete |

### Misc

| Endpoint | Purpose |
| --- | --- |
| `tb_appe.ai.api.list_tools`      | Returns the tool catalog (capability-aware) |
| `tb_appe.ai.api.settings_public` | Public settings: `enabled`, `provider`, `model`, capability flags |
| `tb_appe.ai.api.test_connection` | Pings the configured provider |
...
class AppeBuddyService {
  ...
  Future<Map<String, dynamic>> sendMessage({
    ...
    final resp = await _dio.post(
      '/api/method/tb_appe.ai.api.send_message',
      data: {
        'message': message,
  ...
  Future<List<dynamic>> listConversations({String? status, int limit = 50}) async {
    final resp = await _dio.get(
      '/api/method/tb_appe.ai.api.list_conversations',
  ...
  Future<Map<String, dynamic>> getConversation(String name, {int messageLimit = 100}) async {
    final resp = await _dio.get(
      '/api/method/tb_appe.ai.api.get_conversation',
...
`tb_appe.appe_api.login_user` flow. Set the header:
...
- To add a new tool: register it in `tb_appe/ai/tools.py` via
